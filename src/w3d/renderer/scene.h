...
public:
    ...
    int Get_Scene_ID() override { return 1; }

    void Add_Render_Object(RenderObjClass *obj) { obj->Notify_Added(this); }
    void Remove_Render_Object(RenderObjClass *obj) { obj->Notify_Removed(this); }

    SceneIterator *Create_Iterator(bool onlyvisible) = 0;
    void Destroy_Iterator(SceneIterator *it) = 0;

    void Register(RenderObjClass *obj, RegType for_what) = 0;
    void Unregister(RenderObjClass *obj, RegType for_what) = 0;

    float Compute_Point_Visibility(RenderInfoClass &rinfo, const Vector3 &point) { return 1.0f; }

    void Customized_Render(RenderInfoClass &rinfo) = 0;
    void Post_Render_Processing(RenderInfoClass &rinfo) {}
    
    void Remove_All_Render_Objects();
    void Visibility_Check(CameraClass *);
