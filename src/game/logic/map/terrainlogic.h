...
public:
    ...
    float Get_Bridge_Height(const Coord3D *loc, Coord3D *n) const;

    bool Is_Cell_Entry_Point(const Region2D *cell) const;
    bool Is_Cell_On_End(const Region2D *cell) const;
    bool Is_Cell_On_Side(const Region2D *cell) const;
    bool Is_Point_On_Bridge(const Coord3D *loc) const;

    Drawable *Pick_Bridge(const Vector3 &from, const Vector3 &to, Vector3 *pos) const;
...
{
    Coord3D attack_point1;
    Coord3D attack_point2;
};
...
public:
    ...
    bool Is_Bridge_Repaired(const Object *bridge) const;
    bool Is_Bridge_Broken(const Object *bridge) const;
    void Get_Bridge_Attack_Points(const Object *bridge, TBridgeAttackInfo *attack_info) const;
